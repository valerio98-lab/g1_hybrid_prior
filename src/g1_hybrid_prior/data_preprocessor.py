...
class EndEffectorAugmenter:
    def __init__(
        ...
        if missing_links:
            raise ValueError(f"Link non trovati nell'URDF: {missing_links}\n"
                             f"Link disponibili: {[f.name for f in self.model.frames if 'link' in f.name]}")
        ...
        for csv_idx, joint_name in enumerate(dataset_joints):
            if not self.model.existJointName(joint_name):
                raise ValueError(f"Joint '{joint_name}' presente nel dataset ma NON nell'URDF!")
    ...
    def process_path(self, input_path: str):
        ...
        if path.is_file():
            ...
            output_dir = path.parent / (path.name + "_ee_augmented")
    ...
    def _process_single_file(self, file_path: Path, output_dir: Optional[Path] = None):
            ...
            if len(df.columns) == len(expected_cols):
                ...
                print(f"⚠️ Skipping {file_path.name}: Expected {len(expected_cols)} cols, found {len(df.columns)}")
                return
            ...
            # 4. Loop cinematica
            for i in tqdm(range(n_frames), desc=f"Processing {file_path.name}", leave=False, unit="frame"):
                q[:7] = root_data[i] 
            ...
            if output_dir:
                ...
                save_path = file_path.parent / (file_path.stem + "_end_eff_augmented.csv")
...
def get_args():
    parser = argparse.ArgumentParser(description="G1 End-Effector Augmenter via Pinocchio FK")
    
    # Defaults basati sulla tua configurazione attuale
    DEFAULT_URDF = str(get_project_root() / "assets" / "g1_29dof_with_hand_rev_1_0.urdf")
    DEFAULT_YAML = str(get_project_root() / "config" / "robots.yaml")
    DEFAULT_DATA = str(get_project_root() / "data_raw" / "LAFAN1_Retargeting_Dataset" / "g1")
    
    # Nomi End Effector G1 (default)
    DEFAULT_EES = [
        "left_hand_palm_link", 
    ...
    parser.add_argument("--urdf", type=str, default=DEFAULT_URDF, help="Path to robot URDF")
    parser.add_argument("--yaml", type=str, default=DEFAULT_YAML, help="Path to robots.yaml config")
    parser.add_argument("--data", type=str, default=DEFAULT_DATA, help="Path to csv file or directory")
    parser.add_argument("--robot_name", type=str, default="g1", help="Robot name in yaml config")
    parser.add_argument("--ees", nargs='+', default=DEFAULT_EES, help="List of End-Effector link names")
...
if __name__ == "__main__":
    ...
    try:
        ...
        augmenter = EndEffectorAugmenter(args.urdf, args.ees, cfg)
